import { type array, type describe, type listable, type satisfy } from "@arktype/util";
...
export declare const writeInvalidOperandMessage: <kind extends "regex" | "divisor" | "exactLength" | "max" | "min" | "maxLength" | "minLength" | "before" | "after" | "required" | "optional" | "index" | "sequence" | "structure" | "predicate", expected extends Root<unknown, any>, actual extends Root<unknown, any>>(kind: kind, expected: expected, actual: actual) => `${Capitalize<kind>} operand must be ${import("@arktype/util").join<import("@arktype/util").unionToTuple<import("@arktype/util").isAny<Prerequisite<kind>> extends true ? "any" : import("@arktype/util").isNever<Prerequisite<kind>> extends true ? "never" : unknown extends Prerequisite<kind> ? "unknown" : Prerequisite<kind> extends infer T ? T extends Prerequisite<kind> ? T extends array ? "an array" : T extends object ? import("@arktype/util").describeObject<T> : T extends import("@arktype/util").Stringifiable ? import("@arktype/util").inferDomain<import("@arktype/util").domainOf<T>> extends T ? import("@arktype/util").join<import("@arktype/util").unionToTuple<{
    ...
    string: "a string";
    symbol: "a symbol";
}[import("@arktype/util").domainOf<T>]>, " or ", ""> : never : never>, " or ", "">} (was ${import("@arktype/util").join<import("@arktype/util").unionToTuple<import("@arktype/util").isAny<Exclude<actual["infer"], Prerequisite<kind>>> extends true ? "any" : import("@arktype/util").isNever<Exclude<actual["infer"], Prerequisite<kind>>> extends true ? "never" : unknown extends Exclude<actual["infer"], Prerequisite<kind>> ? "unknown" : Exclude<actual["infer"], Prerequisite<kind>> extends infer T_1 ? T_1 extends Exclude<actual["infer"], Prerequisite<kind>> ? T_1 extends array ? "an array" : T_1 extends object ? import("@arktype/util").describeObject<T_1> : T_1 extends import("@arktype/util").Stringifiable ? import("@arktype/util").inferDomain<import("@arktype/util").domainOf<T_1>> extends T_1 ? import("@arktype/util").join<import("@arktype/util").unionToTuple<{
    boolean: "boolean";
    null: "null";
    undefined: "undefined";
    bigint: "a bigint";
    ...
    string: "a string";
    symbol: "a symbol";
}[import("@arktype/util").domainOf<T_1>]>, " or ", ""> : never : never>, " or ", "">})`;
