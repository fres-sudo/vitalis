...
type LayoutRouteId = RouteId | "/(app)" | "/(app)/help" | "/(auth)/login" | "/(auth)/resetpassword" | "/(auth)/resetpassword/[token]" | "/(auth)/signup" | "/(auth)/test" | "/(auth)/verify/[userId]/[token]" | null
type LayoutParams = RouteParams & { token?: string; userId?: string }
